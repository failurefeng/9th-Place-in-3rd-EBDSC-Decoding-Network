...
def main(cfgs):
    ...
    train_loader, valid_loader, test_loader = AMRDataLoader(dataset=cfgs.data_settings.dataset,
                                                                        ...
                                                                        hdf5_path = cfgs.data_settings.hdf5_path,
    ...
    if cfgs.modes.train:
        ...
        scheduler1 = LinearLR(
            optimizer, 
            start_factor=0.6,  # 初始学习率为设置值的比例
            end_factor=1.0,     # 逐渐升温到完整学习率
            total_iters=warmup_epochs
        )

        # ReduceLROnPlateau主调度阶段
        scheduler2 = ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=3, threshold=0.02, threshold_mode='rel', min_lr=1e-7)

        trainer = Trainer(model=model, model_width=model_width, device=device, optimizer=optimizer, base_lr=cfgs.opt_params.lr,warmup_scheduler=scheduler1, main_scheduler=scheduler2, warmup_epochs=warmup_epochs, criterion=criterion,
                          save_path=cfgs.modes.result_save_path,
                          early_stop=cfgs.opt_params.early_stop,
                          stop_patience = cfgs.opt_params.stop_patience,
                          noise_flag= cfgs.opt_params.noise_flag,
                          mask_flag = cfgs.data_settings.Xmode.options.zero_mask,
                          batch_size = cfgs.opt_params.batch_size,
                          loss_name = cfgs.modes.loss)
        train_loss, train_CE, train_CQ_score, valid_loss, valid_CE, valid_CQ_score = trainer.loop(cfgs.opt_params.epochs, train_loader, valid_loader)
        draw_train(train_loss, train_CE, valid_loss, valid_CE,
                   save_path=cfgs.modes.result_save_path)
    ...
    print("testing start...")
    test_loss, test_CE, test_CQ_score = Tester(model=model, model_width=model_width, device=device, criterion=criterion,
                                                                         batch_size = cfgs.opt_params.batch_size,
