...
5. 理论上，只要保持输入输出一致，你可以使用任意配套的码宽回归网络与相应模型权重来配合解码网络进行训练或测试，另外，由于我的方案中所使用的码宽回归网络在训练时就应用了相位校正，所以在调用它时调用了correct_iq_phase函数来进行相位校正，若你使用自己的码宽回归网络，则可在`solver.py`中删除使用`correct_iq_phase`函数处理输入信号的代码。
