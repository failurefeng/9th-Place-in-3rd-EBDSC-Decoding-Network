...
__all__ = ["loss_CE", "loss_cos", "loss_cos_and_CE", "loss_CE_and_MSE", "loss_Focal", "MixContrastiveLoss", "mixTripletLoss"]
...
class loss_CE(nn.Module):
    ...
    def forward(self, pred, label, ori):
        loss = self.ce(pred, label) #! label 的形状可能是 (batch_size,)，每个值是一个整数（范围是 [0, num_classes - 1]）
        Y_pred = torch.argmax(pred, 1) #! Y_pred 的形状可能是 (batch_size,)，每个值也是一个整数（范围是 [0, num_classes - 1]）
        return loss, Y_pred
...
class loss_cos(nn.Module):
    ...
    def forward(self, targets, preds, mask):
        ...
        # # 对齐 cq_scores 的可导损失
        # loss_cq = self.smooth_cq_loss(cos_sims)
    ...
    def smooth_cq_loss(self, cos_sims):
        lower, upper = 0.7, 0.95
        # 用 sigmoid 软化硬分界
        s_lower = torch.sigmoid(50*(cos_sims - lower))  # 50 为光滑系数，越大分界越陡峭
        s_upper = torch.sigmoid(50*(cos_sims - upper))
        # 线性区间的过渡 (0.7~0.95)
        linear_region = s_lower * (1 - s_upper)  # 仅在 0.7 < cos_sim <0.95 区间激活
        linear_score = 1 * (cos_sims - lower)/0.25 * linear_region
        # 完全达标区间的奖励 (cos_sim >=0.95)
        max_region = s_upper
        max_score = 1 * max_region
        # 总得分（越高越好），转换为损失需取负
        total_score = linear_score + max_score
        loss = - total_score.mean()  # 最小化损失 = 最大化 cq_score
        return loss

class loss_cos_and_CE(nn.Module):
    def __init__(self, alpha=1):
        super(loss_cos_and_CE, self).__init__()
        self.ce = nn.CrossEntropyLoss()
        self.cos = nn.CosineSimilarity(dim=1)
        self.alpha = alpha  # 权重参数，用于平衡余弦相似度和交叉熵损失

    def forward(self, code_seq, pred, mode_soft, mode_label, batch_size):
        """
        计算混合损失函数
        :param code_seq: 真实的码序列 (batch_size, seq_len)
        :param pred: 预测的码序列 (batch_size, pred_seq_len)
        :param mode_soft: 调制类型的预测概率分布 (batch_size, num_classes)
        :param mode_label: 调制类型的真实标签 (batch_size,)
        :return: 总损失, 余弦相似度损失, 交叉熵损失
        """
        # 对每个样本单独处理，因为码序列长度可能不同
        loss_cos_list = []
        for i in range(batch_size):
            # 获取当前样本的真实码序列和预测码序列
            true_seq = code_seq[i]  # (seq_len,)
            pred_seq = pred[i]  # (pred_seq_len,)
            # 截断或补0，使预测序列与真实序列长度一致
            if pred_seq.size(0) > true_seq.size(0):
                pred_seq = pred_seq[:true_seq.size(0)]
            elif pred_seq.size(0) < true_seq.size(0):
                padding_length = true_seq.size(0) - pred_seq.size(0)
                pred_seq = torch.nn.functional.pad(pred_seq, (0, padding_length), "constant", 0)
            # 计算余弦相似度
            cos_sim = self.cos(pred_seq.unsqueeze(0), true_seq.unsqueeze(0))  # 添加批次维度
            loss_cos_list.append(1 - cos_sim)  # 1 - 余弦相似度
        # 计算余弦相似度损失的均值
        loss_cos = torch.mean(torch.stack(loss_cos_list))
        # 计算交叉熵损失
        loss_CE = torch.mean(self.ce(mode_soft, mode_label))
        # 计算总损失
        loss = loss_cos + self.alpha * loss_CE

        return loss, loss_cos, loss_CE
