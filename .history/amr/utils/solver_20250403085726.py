...
class Trainer:
    def __init__(self, model, model_width, device, optimizer, warmup_scheduler, main_scheduler, warmup_epochs, criterion, save_path, valid_freq=1, early_stop=True, stop_patience= 150,  mask_flag=False, batch_size=512, loss_name = None):
        self.model = model
        ...
        #-------------------------记录训练结果------------------------------
        self.result_file = os.path.join(save_path, "training_progress.csv")
        # 初始化时创建CSV文件并写入标题
        if not os.path.exists(self.result_file):
            with open(self.result_file, "w", newline="") as f:
                writer = csv.writer(f)
                writer.writerow(["Epoch", "Train_CQ", "Valid_CQ"])
        #--------------------------------------------------------------------------
    ...
    def loop(self, epochs, train_loader, valid_loader):
        ...
        for ep in range(self.cur_epoch, epochs+1):
            ...
            with open(self.result_file, "a", newline="") as f:
                ...
                writer.writerow([
                    self.cur_epoch,
                    self.train_CQ_score,  # 需要确保这些属性已更新
                    self.valid_CQ_score
                ])
                # 强制刷新写入磁盘
                f.flush()
