    ...
    def _loop_postprocessing(self, CQ_score):
        ...
        if self.best_CQ_score is None and self.cur_epoch > self.rely_epoch+5:
            ...
            if self.counter > self.stop_patience:
                self.stop_flag = True
            os.makedirs(self.save_path, exist_ok=True)
            torch.save({'state_dict': self.model.state_dict()},os.path.join(self.save_path, f"epoch{self.cur_epoch}_{CQ_score}.pth"))
        else:
            self.best_CQ_score = CQ_score
            state['best_CQ_score'] = self.best_CQ_score
            self.best_model_name=f"best_CQ.pth"
            ...
            self.counter = 0
            os.makedirs(self.save_path, exist_ok=True)
            torch.save({'state_dict': self.model.state_dict()},os.path.join(self.save_path, f"best_CQ_epoch{self.cur_epoch}_{CQ_score}.pth"))
