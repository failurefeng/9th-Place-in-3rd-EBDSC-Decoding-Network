...
class Trainer:
    def __init__(self, model, model_width, device, optimizer, warmup_scheduler, main_scheduler, warmup_epochs, criterion, save_path, valid_freq=1, early_stop=True, mask_flag=False, batch_size=512, loss_name = None):
        self.model = model
        ...
        self.train_rounded_CQ_score = None
        self.valid_loss = None
        self.valid_CQ_score = None
        self.valid_rounded_CQ_score = None
        self.scheduler_val_loss = 10
        self.train_loss_all = []
        self.train_CQ_score_all = []
        self.train_rounded_CQ_score_all = []
        self.valid_loss_all = []
        self.valid_CQ_score_all = []
        self.valid_rounded_CQ_score_all = []
        self.valid_freq = valid_freq
        self.save_path = save_path
        self.best_CQ_score = 0.0
        self.best_rounded_CQ_score = None
        self.best_loss = None
        # early stopping
        self.early_stop = early_stop
        self.stop_patience = 150 #! ?个epoch没有提升就停止
        self.delta = 0
        self.counter = 0
    ...
    def loop(self, epochs, train_loader, valid_loader):
        ...
        for ep in range(self.cur_epoch, epochs+1):
            self.cur_epoch = ep
            if self.loss_name == "loss_cos":
                self.train_loss, self.train_CQ_score, self.train_rounded_CQ_score = self.train(train_loader)
                self.train_CQ_score_all.append(self.train_CQ_score)
                self.train_rounded_CQ_score_all.append(self.train_rounded_CQ_score)
            else:
            ...
            if self.loss_name == "loss_cos":
                self.valid_loss, self.valid_CQ_score, self.train_rounded_CQ_score = self.val(valid_loader)
                self.valid_CQ_score_all.append(self.valid_CQ_score)
                self.valid_rounded_CQ_score_all.append(self.valid_rounded_CQ_score)
            else:
                print("in function 'loop()', loss_name is not defined")
            self.valid_loss_all.append(self.valid_loss)
            self._loop_postprocessing(self.valid_CQ_score)

            if self.early_stop and self.stop_flag:
                print(f'early stopping at Epoch: [{self.cur_epoch}]')
                break
        return self.train_loss_all, self.train_CQ_score_all, self.train_rounded_CQ_score, self.valid_loss_all, self.valid_CQ_score_all, self.valid_rounded_CQ_score_all
    ...
    def lr_reset(self, base_lr=3e-4):
        for param_group in self.optimizer.param_groups:
            param_group['lr'] = base_lr * (0.9 ** (self.cur_epoch//20))  # 每次递减20%

    def _iteration(self, data_loader):
        
        iter_loss = AverageMeter('Iter loss')
        iter_CQ_score = AverageMeter('Iter CQ_score')
        iter_rounded_CQ_score = AverageMeter('Iter rounded_CQ_score')
        ...
        for batch_idx, batch in enumerate(data_loader):
            ...
            if (self.model_width is not None) :
                ...
                if  (self.model.training ):
                    alpha = 0
                    std = std = min(0.05 + (1-0.965**self.cur_epoch), 0.8)*0.75  # 指数衰减到稳定
                ...
                mixed_widths = alpha * pred_widths + (1-alpha) * symbol_widths.clone() + width_noise
            
            else:
                if  (self.model.training):
                    std = min(0.05 + (1-0.965**self.cur_epoch), 0.8)*0.75  # 指数衰减到稳定
                else:
                    std = (0.6-min(0.05 + (1-0.965**self.cur_epoch), 0.8)*0.75)
                
                width_noise = (torch.randn(1, device=self.device) * 0.05 +torch.abs(torch.randn(1, device=self.device)) * 0.05 * 0.1) * std                 
                mixed_widths = symbol_widths.clone() + width_noise
                # # 随机扰动 symbol_width, 现在看来好像用不太上---------------------------------------
                # period = 'train' if self.model.training else 'valid'
                # if self.noise_flag:
                #     width_rand_factor = 0.5
                #     if period == 'train':
                #         std = width_rand_factor * (1.0 - 0.985 ** self.cur_epoch)  # 指数衰减到稳定
                #     elif period == 'valid':
                #         std = width_rand_factor * (1.0 - 0.985 ** self.cur_epoch) *0.2 # 指数衰减到稳定
                #     if (period == 'train')| (period == 'valid'): # 随机扰动 symbol_width
                #         symbol_widths = symbol_widths.clone() + (torch.randn(1, device=self.device) * 0.05 +torch.abs(torch.randn(1, device=self.device)) * 0.05 * 0.1) * std 
                #--------------------------------------------------------------------------------
            ...
            if self.loss_name == 'loss_cos': #! 开始前向传播
                with autocast():
                    C_pred = self.model(S, split_Mask)
                    if self.model.training:
                        assert C_pred.requires_grad, "模型输出未保留梯度!"
                    C_pred = C_pred.to(self.device)
                    loss, CQ_sore, rounded_CQ_score = self.criterion(padded_C, C_pred, code_Mask)
            else:
            ...
            iter_CQ_score.update(CQ_sore)
            iter_rounded_CQ_score.update(rounded_CQ_score)
        ...
        if not self.model.training: # 验证阶段统一调度
            ...
            if self.cur_epoch %15 ==0 and self.cur_epoch >20:
                self.lr_reset()
        ...
        if self.loss_name == "loss_cos":
            if self.model.training:
                print(f'Train | '
                            ...
                            f'rounded_CQ_score: {iter_rounded_CQ_score.avg:.4f} | '
                            f'Lr: {iter_lr.avg:.4e} | '
                            f'alpha:{alpha} | '
                            f'noise_std:{std} | '
        ...
        return iter_loss.avg.item(), iter_CQ_score.avg.item(), iter_rounded_CQ_score.avg.item()
    ...
    def _save(self, state, name):
        ...
        print(f"best CQ_score update to {self.best_CQ_score:.4f},model saved to {full_path}.")
    def get_best_model_path(self):
        return os.path.join(self.save_path, self.best_model_name)

    def _loop_postprocessing(self, CQ_score):
        self.last_valid_CQ_score = CQ_score
        state = {
            'epoch': self.cur_epoch,
            'state_dict': self.model.state_dict(),
            'optimizer': self.optimizer.state_dict(),
            'best_CQ_score': self.best_CQ_score
        }
        if self.best_CQ_score is None and self.cur_epoch > self.rely_epoch+5:
            self.best_CQ_score = CQ_score
            state['best_CQ_score'] = self.best_CQ_score
            self.best_model_name=f"best_CQ.pth"
            self._save(state, name=self.best_model_name)
        elif CQ_score < self.best_CQ_score:
            self.counter += 1
            if self.counter > self.stop_patience:
                self.stop_flag = True
            os.makedirs(self.save_path, exist_ok=True)
            torch.save({'state_dict': self.model.state_dict()},os.path.join(self.save_path, f"epoch{self.cur_epoch}_{CQ_score:.4f}.pth"))
        else:
            self.best_CQ_score = CQ_score
            state['best_CQ_score'] = self.best_CQ_score
            self.best_model_name=f"best_CQ.pth"
            ...
            self.counter = 0
            os.makedirs(self.save_path, exist_ok=True)
            torch.save({'state_dict': self.model.state_dict()},os.path.join(self.save_path, f"best_CQ_epoch{self.cur_epoch}_{CQ_score:.4f}.pth"))
...
class Tester:
    ...
    def __call__(self, test_loader):
        self.model.eval()
        with torch.no_grad():
            loss, CQ_score, rounded_CQ_score = self._iteration(test_loader)

        return loss, CQ_score, rounded_CQ_score
    
    # def _save_debug_csv(self):
    #     if not self.debug_data:
    #         return

    #     csv_path = "code_prediction_debug.csv"
    #     fieldnames = ["sample_id", "true_sequence", "rely_pred_seq", 
    #                  "rely_rounded_CQ_score", "symbol_width", "pred_width"]

    #     with open(csv_path, 'w', newline='') as csvfile:
    #         writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
    #         writer.writeheader()
    #         for sample in self.debug_data:
    #             writer.writerow({
    #                 "sample_id": sample["sample_id"],
    #                 "true_sequence": sample["true_sequence"],
    #                 "rely_pred_seq": sample["rely_pred_seq"],
    #                 "rely_rounded_CQ_score": f"{sample['rely_rounded_CQ_score']:.1f}",                    
    #                 "symbol_width": f"{sample['symbol_width']:.2f}",
    #                 "pred_width": f"{sample['pred_width']:.2f}",
    #                 })
    #     print(f"调试样本数据已保存至: {os.path.abspath(csv_path)}")
    ...
    def _iteration(self, data_loader):
        ...
        iter_rely_rounded_CQ_score = AverageMeter('Iter rely_rounded_CQ_score')
        stime = time.time()
        for batch_idx, batch in enumerate(data_loader):
            ...
            if self.model_width is not None:
                ...
                if self.loss_name == 'loss_cos':
                    with autocast():
                        ...
                        rely_loss, rely_CQ_score, rely_rounded_CQ_score = self.criterion(padded_C, rely_C_pred, code_Mask)
                else:
                    print(f"Invalid model_name value: {self.loss_name}")
                    raise ValueError("Invalid loss_name value")
                iter_rely_loss.update(rely_loss)
                iter_rely_CQ_score.update(rely_CQ_score)
                iter_rely_rounded_CQ_score.update(rely_rounded_CQ_score)
            #-----------------------------------------------------------------------------------------------------------------
            ...
            if self.loss_name == 'loss_cos':
                with autocast():
                    C_pred = self.model(S, split_Mask)
                    C_pred =  C_pred.to(self.device)
                    loss, CQ_score, rounded_CQ_score = self.criterion(padded_C, C_pred, code_Mask)
            else:
                print(f"Invalid model_name value: {self.loss_name}")
                raise ValueError("Invalid loss_name value")
            ...
            # # Debug 采集样本------------------------------------------------
            # if self.is_debug_sample :
            #     # 对预测结果进行后处理
            #     processed_preds = rely_C_pred.round().clamp(0, 31).int().cpu().numpy()

            #     # 获取有效长度
            #     seq_lens = code_Mask.sum(dim=1).cpu().numpy()  # 假设mask中0表示有效位置

            #     # 转换为可读字符串
            #     for i in range(min(4, C_pred.size(0))):
            #         seq_len = int(seq_lens[i])
                    
            #         # 真实序列（去除填充）
            #         true_seq = padded_C[i].cpu().numpy()[:seq_len].astype(int)
                    
            #         # 原始预测和四舍五入后的预测
            #         rely_pred_seq = processed_preds[i][:seq_len]
                    
            #         _ , _ , singe_rely_rounded_CQ_score = self.criterion(padded_C[i].unsqueeze(dim=0), C_pred[i].unsqueeze(0), code_Mask[i].unsqueeze(0))

            #         # 记录样本信息
            #         sample_debug = {
            #             "sample_id": f"batch{batch_idx}_sample{i}",
            #             "true_sequence": " ".join(map(str, true_seq.tolist())),
            #             "rely_pred_seq": " ".join(map(str, rely_pred_seq.tolist())),
            #             "rely_rounded_CQ_score": singe_rely_rounded_CQ_score,
            #             "symbol_width": symbol_widths[i].item() if symbol_widths is not None else 0,
            #             "pred_width": pred_widths[i].item() if pred_widths is not None else 0,
            #         }
            #         self.debug_data.append(sample_debug)

            #         # 提前停止收集样本
            #         if len(self.debug_data) >= self.debug_sample_num:
            #             break
        ...
        if self.loss_name == "loss_cos":
            ...
            if self.model_width is not None:
                print('Test | '
                        ...
                        f'rely_rounded_CQ_score: {iter_rely_rounded_CQ_score.avg:.4f} | ')
        else:
            print("in _iteration, Invalid model_name value")
        return iter_loss.avg.item(), iter_CQ_score.avg.item(), iter_rounded_CQ_score.avg.item()
