...
class Trainer:
    ...
    def _iteration(self, data_loader):
        ...
        for batch_idx, batch in enumerate(data_loader):
            ...
            if (self.model_width is not None) :
                ...
                if not self.model.training and self.cur_epoch > self.rely_epoch:
                    if self.cur_epoch < 30:
                        alpha = min(0.1+self.cur_epoch/30, 1.0) * 0.3
                    else:
                        alpha = 0.32
                else:
                    alpha = min(0.05+self.cur_epoch/50, 1.0)
        ...
        if not self.model.training: # 验证阶段统一调度
            ...
            if self.cur_epoch %20 ==0 and self.cur_epoch >30:
                self.lr_reset()
    ...
    def _loop_postprocessing(self, CQ_score):
        ...
        if self.best_CQ_score is None and self.cur_epoch > self.rely_epoch+5:
            ...
            self.best_model_name=f"best_CQ_epoch{self.cur_epoch}.pth"
            ...
            self.counter = 0
            os.makedirs(self.save_path, exist_ok=True)
            torch.save({'state_dict': self.model.state_dict()},os.path.join(self.save_path, f"epoch{self.cur_epoch}.pth"))
        
        if CQ_score > self.last_CQ_score+0.01:
            self.counter = 0
