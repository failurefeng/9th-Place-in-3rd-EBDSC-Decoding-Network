...
def main(cfgs):
    ...
    if cfgs.modes.train:
        ...
        scheduler1 = LinearLR(
            optimizer, 
            start_factor=0.5,  # 初始学习率为设置值的比例
            end_factor=1.0,     # 逐渐升温到完整学习率
            total_iters=warmup_epochs
        )

        # ReduceLROnPlateau主调度阶段
        scheduler2 = ReduceLROnPlateau(optimizer, mode='min', factor=0.7, patience=2, threshold=0.0001, threshold_mode='rel', min_lr=1e-6)
        scheduler3 = CosineAnnealingLR(optimizer, T_max=cfgs.opt_params.epochs, eta_min=1e-6)

        trainer = Trainer(model=model, model_width=model_width, device=device, optimizer=optimizer, warmup_scheduler=scheduler1, main_scheduler=scheduler3, warmup_epochs=warmup_epochs, criterion=criterion,
