...
3. 修改设置: 在 `configs/transformerlstm_ours.yaml` 中修改相应参数，然后在终端运行 `python main.py`来获取结果，复现我的解码网络需要分三步进行训练，如下所示（我三个阶段训练得到的模型分别放在`results\demod\s1`、`results\demod\s2`、`results\demod\s3`）：
